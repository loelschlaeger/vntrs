...
"A Heuristic for Nonlinear Global Optimization". The method combines
neighborhood exploration with a trust-region framework to search the solution
space efficiently. It can terminate a local search early when the iterates
converge toward a previously visited local optimum or when further improvement
within the current region is unlikely. The algorithm can also be used to
...
1. Specify a function `f` that computes the objective value. It may also return
   the gradient and Hessian. Return either a numeric value or a named list with
   `value`, `gradient`, and `hessian` components. Omitted derivatives are
...
The example below minimizes the well-known [six-hump camel function](https://www.sfu.ca/~ssurjano/camel6.html), which has two global minima.
