...
1. Specify a function `f` that computes value, gradient, and Hessian of the objective to be optimized and returns them as a named list with elements `value`, `gradient`, and `hessian`. Note that `gradient` and/or `hessian` can be unspecified, in which case finite differences are used.
